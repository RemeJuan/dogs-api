...
export class AuthController {
  ...
  @ApiResponse({ status: 503, description: 'Service unavailable - Authentication service is down' })
  ...
  @ApiOperation({
    ...
    description: 'Returns the currently authenticated user based on the provided access token'
  })
  @ApiHeader({
    name: 'Authorization',
  ...
  @ApiResponse({ status: 200, description: 'User information retrieved successfully', type: UserDto })
  @ApiResponse({ status: 400, description: 'Bad request - Missing or invalid authorization header' })
  @ApiResponse({ status: 401, description: 'Unauthorized - Invalid or expired token' })
  @ApiResponse({ status: 403, description: 'Forbidden - Token is not valid' })
  @ApiResponse({ status: 500, description: 'Internal server error' })
  @ApiResponse({ status: 503, description: 'Service unavailable' })
  async getCurrentUser(@Headers('authorization') authorization: string): Promise<UserDto> {
  ...
  @ApiResponse({ status: 400, description: 'Bad request - Invalid refresh token format' })
  @ApiResponse({ status: 401, description: 'Unauthorized - Invalid or expired refresh token' })
  @ApiResponse({ status: 403, description: 'Forbidden - Refresh token is not valid' })
  @ApiResponse({ status: 500, description: 'Internal server error' })
  @ApiResponse({ status: 503, description: 'Service unavailable' })
  async refreshToken(@Body() request: RefreshTokenDto): Promise<RefreshTokenResponseDto> {
