...
export class ImagesController {
  ...
  @ApiOperation({
    ...
    description: 'Returns random images for a specific dog breed',
  })
  @ApiParam({
    name: 'breed',
  ...
  @ApiResponse({
    status: 400,
    description: 'Bad request - Invalid breed name or count parameter',
  })
  @ApiResponse({ status: 404, description: 'Not found - Breed does not exist' })
  @ApiResponse({
    status: 500,
    description:
      'Internal server error - Failed to fetch images from external API',
  })
  @ApiResponse({
    status: 503,
    description: 'Service unavailable - External API is down',
  })
  async getBreedImages(
    @Param('breed') breed: string,
    @Query('count', new ParseIntPipe({ optional: true })) count = 3,
